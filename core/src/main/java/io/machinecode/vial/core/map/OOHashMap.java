...
public class OOHashMap<K, V> implements OOMap<K, V>, Serializable {
  ...
  public V get(final Object key) {
    if (key == null) {
      return this._haveNoValue ? this._noValue : null;
    }
    final Object[] data = this._data;
    final int nm = this._nextMask;
    final int hash = _spread.spread(key.hashCode());
  ...
  public V getOrDefault(final Object key, final V defaultValue) {
    if (key == null) {
      return this._haveNoValue ? this._noValue : defaultValue;
    }
    final Object[] data = this._data;
    final int nm = this._nextMask;
    final int hash = _spread.spread(key.hashCode());
  ...
  private void _remove(int index) {
    ...
    for (; ; ) {
      ...
      int slot = (hash & im) << 1;
      if (index <= next ? index >= slot || slot > next : index >= slot && slot > next) {
        data[index] = key;
  ...
  public boolean replace(final K key, final V oldValue, final V newValue) {
    if (key == null) {
      if (!this._haveNoValue
          || (oldValue == null ? _noValue != null : !oldValue.equals(_noValue))) {
        return false;
      }
