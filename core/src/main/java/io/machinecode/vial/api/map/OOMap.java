...
public interface OOMap<K, V> extends Map<K, V>, Iterable<OOCursor<K, V>> {
  ...
  /**
   * Analogous to {@link #put(Object, Object)} but does not return the previous mapping. Instead it
   * returns this map to allow it to be chained after initialization.
   *
  ...
  /**
   * Remove a single associated of the provided value. An implementation MUST release the references
   * to both the key and value from the removed associated to make them available for garbage
   * collection should no other references exist.
   *
   ...
   * @return The value associated with the key, or {@code null} if the key is not associated with a
   *     value.
   */
   ...
   * @param value The value the key's associated value must be {@link #equals(Object)} with for the
   ...
   * @param oldValue The value the key's associated value must be {@link #equals(Object)} with for
  ...
  /**
   * An implementation SHOULD change the size of the underlying storage to be able to accommodate
   * the desired number of elements. This method may be used to either grow or shrink the map if the
   * desired number of elements is greater than or less than the elements in the map.
   *
   * <p>Calling {@code map.capacity(0);} on a compliant implementation after removing elements
   * SHOULD ensure that the map releases any excess resources.
   *
   * <p>Calling with a larger size SHOULD preallocate enough storage that the desired number of keys
   * may be associated with values without allocating additional storage.
   *
   * @param desired The amount of elements the map SHOULD be able to contain without allocating
   *     additional storage.
   * @return This map for method chaining.
   */
